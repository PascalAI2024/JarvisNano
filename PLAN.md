...
| N11.4 | The worker lives in the gateway (owner, 2026-09-02: "scrap the devbox, use the sandbox agent in JarvisMCP") | BUILT 2026-09-02: `board_poll` claims one item per poll and settles it in the same call — a repo goal → `sandboxes.workerSubmit` (Pi, branch delivery), anything else → `memory.search` context + `research` + `memory.capture`; measured 21 s on the live board; blocked with the reason on any error | One spoken job with no repo: completed and announced within two polls, the full cited answer in the company brain under `jarvisnano-desk`; one spoken job naming an allowlisted repo: a branch delivered and announced; an unlisted repo blocks the item with `repo_not_allowed`; a poll killed at 30 s leaves an expired lease that a later poll recovers |
