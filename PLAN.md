...
|---|---|---|---|
| N14.1 | **The dial timer no longer caps the watch at 8 fps** | `frames/4 s` on a baked-dial style follows the cost model (≥ 10 fps at 240 MHz) instead of sitting at 8.0–8.4 | `face_fps(JR_FACE_DIAL_*)` was 8; the anim timer's tick is the only invalidation source (`gfx_anim.c` timer → `gfx_obj_invalidate`), so the four dials flushed 8 frames a second whatever the ladder asked. Now 24, the ladder still slows rest. **Shipped 2026-09-05 late** |
