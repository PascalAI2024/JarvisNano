...
| N13.15 | **`POST /api/debug/briefing`** delivers the morning glance now, any hour, without touching the once-a-day latch (a flag the voice task takes; dev-gated) | BUILT | Log `briefing: spoken, forced (...)` with the composed text; the device speaks it |
| N13.13 | **WATCH ≥ 17 fps on every style** (N12.6). Four levers pulled (render clock on `/api/display`, per-frame hoist, shadows only awake, AA 8→4 on wide hands), one reverted (outline fold: null, 932 B). **Measured at 240 MHz unmuted with a session open: 7–12 fps — NOT MET.** The overlay is 17–61 ms of a 51–92 ms frame; the engine's per-strip dial decode + flush is the rest and does not scale with the clock. Next lever is the engine's: decode a resident dial once into a PSRAM RGB565 frame and blit it; a static dial skips the face path | NOT MET — finding recorded | Six styles ≥ 17 fps AWAKE at 240 MHz from `/api/cockpit`; `render_us/frames` per style in the evidence; hud + shell suites green; JARVIS checksums unchanged |
