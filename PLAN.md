...
Current plan, reconciled **2026-09-05**. This file contains only active work and
...
| N12.2 | Hands drawn like hands | anti-aliased tapered polygons, bevel, outline, shadow; 360-angle probe shows no combing; JARVIS checksum unchanged; sweeping seconds | BUILT 88e30164: 13 hands × 360° contiguous, JARVIS checksum pinned, sweep from a per-frame ms phase |
| N12.3 | Baked dials from the concept art | `dial_diver/dress/pilot/future.eaf` from the owner's picks (Codex 01 TL, 02 TL, 03 TL, 05 TR); a missing clip falls back to the procedural dial, one log line | in flight — **SPIFFS is full**: 4728 KB of faces + 852 KB of dials against ≈ 5170 KB usable (SPIFFS keeps 2 of 16 pages per block for lookup). Ship what fits in priority FUTURE, PILOT, DIVER, DRESS. **Resolved 14d65f31:** per-block RAW/RLE (decoder reads the enc byte per block) brought the four to 624 KB; all four staged, 128–144 KB of partition left, spiffsgen arithmetic in `docs/reference/asset-pipeline.md`. The next clip needs N12.5 |
| N12.4 | Complications read the device | PILOT: seconds / battery % / today's °F on baked sub-dials; DIVER: live date in the window; FUTURE: date, weather glance, battery, link in the baked cells; nothing invented, stale weather dimmed | BUILT 88e30164, seen on the glass: `WED 02 SEP`, `77° OVERCAST 87/76`, `100%`, `MUTED`; DIVER `02`; PILOT three needles |
...
| N12.6 | Proof on the panel | six framebuffers in `docs/evidence/20260902-watch2-*.png` plus one photograph of the panel; AWAKE fps on WATCH ≥ 17; PSRAM free ≥ 1 MB with all resident dials | PARTIAL: six framebuffers in evidence, PSRAM 4.15 MB free with four dials resident; **fps gate NOT met** — render cost per frame at 160 MHz JARVIS 69 ms · DRESS 89 · DIVER 108 · MINIMAL 109 · PILOT 112 · FUTURE 127, ≈ 17/14/14/13/12 fps at 240 (`/api/diag/tasks` deltas; awake could not be held under privacy). Levers: cache the AA hand coverage per angle bucket, draw the dial once per second when nothing but the seconds hand moved, drop the shadow at the rest rung. The panel veil bug (dials at half brightness) was found from these captures and fixed |

## Wave N13 — polish, then push (2026-09-05)

Owner: *"polish and enhance … anything else the team comes up with to make
this a pleasure to use, no fluff, something useful and unique for me, and
push the limits of this hardware"*, plus the three offered enhancements.
A read-only baseline over Wi-Fi came first and found seven things the
2026-09-02 handoff did not know; the fixes went in before the features.
Evidence for every row: `docs/evidence/20260905-*`, the numbers in
`docs/NEXT_SESSION.md`.

| # | Item | Status | Acceptance |
|---|---|---|---|
| N13.1 | **A privacy long-press cost every reply 300 ms.** The mute earcon went through the reply-underrun accounting: four "holes mid-reply", `replies 1`, pre-roll 600 → 900 with no Gemini reply ever played (`docs/evidence/20260905-earcon-preroll.log`). Now `pb_enqueue` counts outstanding earcon samples and the feeder books nothing while they drain (+80 ms tail); tones are rendered whole so they cannot starve their own ring; the walk is a pure `jr_dsp` stepper with eight host tests (`host/test_jitter.c`; the mutation reverting the guard fails three) | BUILT | Reset the counters, long-press mute then unmute: `playback.replies 0, underruns 0`, no `hole mid-reply` in the log, pre-roll stays 600; one spoken turn then reads `replies 1` |
| N13.2 | **The brightness fade wrote fifteen log lines.** `esp_log_level_set("co5300_spi", ESP_LOG_WARN)` at the log-ring install; 97 of 271 lines in a 16 KB tail had been the driver's per-step INFO | BUILT | After a mood cycle the 16 KB tail has zero `set brightness` lines |
| N13.3 | **The FUTURE weather cell cut a word** (`75* LIGHT DRIZZ`, `docs/evidence/20260905-future-cut-word.png`). `watch_cells` drops the qualifier word, then cuts at a word boundary with the "." mark; shell test with a mutation check | BUILT | Panel reads `75* DRIZZLE` (or the day's word) with no mid-word cut |
| N13.4 | **A slow finger on the volume rail muted the device.** `TOUCH_HOLD_SLOP_PX` 48 > `TOUCH_SWIPE_MIN_TRAVEL_PX` 42, so 43–48 px of drift was "still a hold" and the mid-hold timer beat the swipe classifier; the privacy consumer checked no coordinates. Now the three slops agree at 42, a hold that starts on the rim (r ≥ 168) is ignored with the neutral ack, and `gesture-doctor` asserts `hold_slop <= swipe_min` | BUILT | Synthetic hold at (47,101) with 42 px of drift changes the volume and leaves `privacy` alone; a centre hold by finger still logs `gesture: long-press mute` |
| N13.5 | **`jr_pb_feed` had 524 bytes of stack.** The 1536-byte `chunk[]` local is a static internal buffer; stack size unchanged | BUILT | `/api/diag/tasks` `jr_pb_feed stack_free ≥ 1500` after a spoken turn and an earcon; largest internal block unchanged |
| N13.6 | **The scripts run on Windows.** PIL before `sips`, a host-bound `JARVIS_PAIRING_TOKEN` env fallback where there is no macOS keychain, UTF-8 stdout in `gesture-doctor`, `tempfile` instead of `/tmp`, no `termios` at import, a one-line message instead of a traceback without a host; 37 unittest cases | DONE | `jarvisctl screen`, `gesture-doctor --host`, and the unittest all exit 0 on the Windows desk |
| N13.7 | **One host-tests entry.** `scripts/host-tests.sh` runs all five suites (hud, shell, core, tool templates, desk CLI) with positive counts, and takes a Docker lane (`jarvisnano-hosttests`, `scripts/host-tests.Dockerfile`) when there is no `cc`; Git Bash's `/project` → `C:/Program Files/Git/project` rewrite is defeated by `MSYS_NO_PATHCONV=1` in `build-v5.sh` (`docs/reference/build-toolchain.md`). Closes N6.6 | DONE | `./scripts/host-tests.sh` → `suites run: 5 failed: 0` on this machine and inside Docker |
| N13.8 | **The sun and the hours in the glance.** `weather_glance` also asks Open-Meteo for today's sunrise/sunset and 36 hourly rain probabilities in its own `try` (226 bytes, measured); the device keeps them beside the weather; an AWAKE/AMBIENT device refreshes every 30 min from any screen so the FUTURE cell and the rain latch stay honest (WHISPER/DREAM never fetch) | BUILT | `/api/cockpit` shows the sun times; the log shows `weather: fetch submitted (half hour)` while lit and never at rest |
| N13.9 | **Rain in the next three hours, once per front.** `jr_rain_warning_step` (pure, `jr_core/glance.c`, six host tests with the real 2026-09-05 hourly answer): warns once when any of the next three hours reaches 60 %, re-arms only when the window drops under 30 %. `RAIN IN 2H` caption when muted, spoken with an open session, always a `RAIN` ACTIVITY row | BUILT | A staged wet window produces one caption and one row, and the next fetch is silent |
| N13.10 | **The day arc on FUTURE and PILOT.** `jr_display_sun_set` publishes the sun at 1 Hz beside the clock; the two data dials draw daylight as a thin gold band on a 24-hour scale (noon at 12) with a dot for now; the other four dials stay as designed | BUILT | Shell test pins the arc's angles for 07:02/19:35 and "no sun → nothing"; panel capture shows the band on FUTURE |
| N13.11 | **The morning glance speaks.** The first lift after a rest (or a deep-sleep wake by lift/touch), once per calendar day in 05:00–11:00, earns one briefing composed only of what the device holds — date, weather, rain within 12 h, delegated tasks finished since the last briefing, battery under 30 % off USB — as one text turn through `handle_say`; muted, a `GOOD MORNING 75* DRIZZLE 2 DONE` caption. The once-a-day latch lives in NVS (`brief_yday`) because the morning lift is usually a fresh boot. Pure `jr_briefing_*` in `jr_core/glance.c` with host tests | BUILT | Rest ≥ 5 min, lift in the window → `briefing: spoken` and a reply naming the date and the temperature; a second lift → nothing; muted → the caption |
| N13.12 | **A chime when a delegated task lands.** `jr_audio_play_chime` (raised-cosine notes) plays E5–G5–B5 in `board_announce()` before the caption or the spoken line; at WHISPER at half volume; never over a reply | **PROVEN 23:50** — `chime: done` then `board: done …` and the `DONE:` caption, `playback.replies` unchanged | Complete an item from the desk: within 90 s the glass shows `DONE: …`, the chime is heard, `playback.replies` is unchanged |
| N13.14 | **The board poll can outrun its own cap.** Creating an item from the desk showed the gateway completing it in 45 s while two device polls answered `http_error 500` at 31 s (the poll researches inside the call); the announcement came from the recovery path two polls later. The device is right to treat a 500 as a failed poll, but a poll that does the work and then times out is work done twice | FINDING | Either the gateway answers the poll before it researches (claim, return, research on its own clock, complete), or the device's poll timeout is raised above the gateway's research budget; one announcement per item either way |
| N13.15 | **`POST /api/debug/briefing`** delivers the morning glance now, any hour, without touching the once-a-day latch (a flag the voice task takes; dev-gated) | **PROVEN 23:52** — muted: `GOOD MORNING 76* OVERCAST` caption; unmuted: the composed turn spoken ("It is Saturday 5 September. Fort Lauderdale is 76 degrees and overcast, high 91, low 76…"), `replies 1` | Log `briefing: spoken, forced (...)` with the composed text; the device speaks it |
| N13.13 | **WATCH ≥ 17 fps on every style** (N12.6). Four levers pulled (render clock on `/api/display`, per-frame hoist, shadows only awake, AA 8→4 on wide hands), one reverted (outline fold: null, 932 B). **Measured at 240 MHz unmuted with a session open: 7–12 fps — NOT MET.** The overlay is 17–61 ms of a 51–92 ms frame; the engine's per-strip dial decode + flush is the rest and does not scale with the clock. Scouted the same evening: the frame is overlay + engine CPU + 21.7 ms DMA + a forced tick delay, serialized, AND the dial's own anim timer capped every baked dial at 8 fps — see wave N14 | NOT MET — finding recorded, N14 scouted | Six styles ≥ 17 fps AWAKE at 240 MHz from `/api/cockpit`; `render_us/frames` per style in the evidence; hud + shell suites green; JARVIS checksums unchanged |


## Wave N14 — the watch off the engine (scouted 2026-09-05, not started)

The scout of the engine's per-strip pipeline (`managed_components/espressif2022__esp_emote_gfx`, `gfx_render.c`, `gfx_anim.c`; read-only, cited) explains the N13.13 numbers and names the levers in order of cost. The frame on WATCH is, measured and modelled to within a frame or two of `actual_fps`: **overlay + engine CPU + 21.7 ms QSPI DMA + up to 10 ms of forced tick delay**, serialized.

| # | Item | Acceptance | Notes |
|---|---|---|---|
| N14.1 | **The dial timer no longer caps the watch at 8 fps** | `frames/4 s` on a baked-dial style follows the cost model (≥ 10 fps at 240 MHz) instead of sitting at 8.0–8.4 | `face_fps(JR_FACE_DIAL_*)` was 8; the anim timer's tick is the only invalidation source (`gfx_anim.c` timer → `gfx_obj_invalidate`), so the four dials flushed 8 frames a second whatever the ladder asked. Now 24, the ladder still slows rest. **Shipped and measured 2026-09-06 00:0x:** DRESS 8.2 → 12.0 fps, DIVER 8.4 → 10.5, JARVIS 11.4 (no cap), FUTURE/PILOT/MINIMAL unchanged at 7.6–8.7 because their frame is already longer than 1/8 s (`docs/evidence/20260905-watch-fps.md`) |
| N14.2 | **Take the dial off the engine.** Decode the resident `JR_FACE_DIAL_*` clip once (`eaf_frame_decode`, one call, panel byte order) into a 434 KB PSRAM RGB565 frame, hide the anim on WATCH, blit the frame per strip (the `apply_canvas` memcpy is the shape) and drive the cadence with `gfx_disp_refresh_all` from `display_task` | DIVER ≈ 98 → ~69 ms/frame (~14.5 fps), DRESS ~18 fps, FUTURE ~13.7; PSRAM ≥ 1 MB free with one shared decoded frame | Removes the engine's per-frame CPU (436 KB bg memset + 224 KB block decode + 434 KB palette expand ≈ 37 ms on DIVER). One shared buffer re-decoded on style change; four resident (1.74 MB) only when PSRAM is above 3 MB. Decode off the render task; the refresh must be unconditional on WATCH or the glass freezes silently |
| N14.3 | **Move the DMA wait one strip later** (engine patch via `scripts/patch-v5-managed.py`, the established mechanism) | ~21.7 ms/frame back on every screen: DIVER ~47 ms (21 fps), DRESS ~34 ms (29 fps) | `gfx_render.c` clears `WAIT_FLUSH_DONE`, calls `flush_cb`, then blocks on **this** strip's DMA before filling the next; the double buffer is allocated and never exploited. The event-group clear must move with the wait or a stale bit lets a strip be overwritten mid-DMA; `snapshot_record_flush` and the ISR error paths must still pair one set-bit per submission |
| N14.4 | **Drop the forced tick delay** | ~10 ms/frame | `gfx_cal_task_delay` floors the post-render delay at `1000/configTICK_RATE_HZ + 1` = 11 ms with `CONFIG_FREERTOS_HZ=100`: 0–10 ms of idle after every frame. Either patch the floor or raise the tick rate (a `sdkconfig.defaults` change with its own costs) |

The QSPI ceiling itself is 40 MHz × 4 lanes = 20 MB/s: 21.7 ms a frame, ~46 fps, and it does not scale with the CPU clock. `JR_DISPLAY_RENDER_FPS 24` already notes the measured ~23 fps full-frame ceiling.
