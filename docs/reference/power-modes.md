...
stateDiagram-v2
    ...
    AWAKE --> WHISPER: muted · still 5 s (the cable no longer holds it)
...
| 2026-09-02 | **Muted is a watch.** With the microphone off by the owner's hand the ladder takes `quiet` (`jr_mood_in_t`, from `s_voice_privacy_paused`): five seconds still (`JR_MOOD_QUIET_MS`) and it is WHISPER — the dim watch at brightness 22, 6 fps, the 160 MHz gear — with AMBIENT skipped and USB no longer counted as busy. Motion, a touch, a live phase and face-down keep their meanings, DREAM and sleep keep their clocks, so this is not the 2026-08 mute-forces-DREAM-at-8 mistake in new clothes: a muted device that is asking stays readable. Pinned by `test_mood_quiet_rests_as_a_watch_in_five_seconds` (mutation-checked). Owner: "when in mute mode should go in low power mode with watch with low brightness" | `components/jr_core/src/mood.c`; `main/main.c` mood tick |
