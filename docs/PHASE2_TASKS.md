...
- `[~]` HTTP reachability: isolate why Mac curls to `192.0.2.80` time out despite ARP resolution; after the 2026-05-03 flash, `/api/status`, `/api/battery`, and `/api/camera/snapshot` still timed out from macOS.
