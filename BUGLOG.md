...
| B5 | Device goes silently deaf ("no response") | Tap = privacy mute (trap); stuck challenge flag; no re-arm coverage | Challenge+re-arm FIXED on device; **tap redesign CODED, in pending flash** — until then taps still mute (hit again tonight: 65 taps, privacy_paused=true, un-muted remotely twice) |
| B6 | Wake didn't respond | Was correct behavior: device was tap-muted (B5); wake honors deliberate mutes | Resolves with B5 pending flash |
| B7 | Watch↔face transition "not smooth and weird" | Clock overlay pops (instant full dim, no easing) | IN PROGRESS — glass-ux teammate, jr_display |
| B8 | Swipes/double-tap do nothing | Never implemented | CODED (glance/peek/attention), in pending flash |
